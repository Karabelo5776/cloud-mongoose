...
const QueryManagement = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const transformedStats = {
                totalQueries: statsRes.data.totalQueries || 0,
                pendingQueries: statsRes.data.pendingQueries || 0,
                completedQueries: statsRes.data.completedQueries || 0,
                autoReplied: statsRes.data.autoReplied || 0
            };
    ...
    useEffect(() => {
        if (chartRef.current && stats.totalQueries > 0) {
                ...
                data: {
                    ...
                    datasets: [{
                        data: [
                            ...
                            stats.completedQueries - stats.autoReplied,
                            stats.autoReplied
                        ],
                        backgroundColor: [
                            "#FF5733", // Red for pending
                            "#33FF57", // Green for completed
                            "#3380FF"  // Blue for auto-replied
                        ],
                        borderWidth: 1
                    }]
                },
    ...
    const handleRespond = async (queryId) => {
        ...
        try {
            ...
            // Clear the response for this query
            setResponses(prev => {
            ...
            <div className="query-stats-section">
                ...
                <div className="query-stats-grid">
                    ...
                    <div className="query-stats-card pending">
                        <h4>Pending</h4>
                        <p>{stats.pendingQueries}</p>
                    </div>
                    <div className="query-stats-card completed">
                        <h4>Completed</h4>
                        <p>{stats.completedQueries}</p>
                    </div>
                    <div className="query-stats-card auto-replied">
                        <h4>Auto-Replied</h4>
                        <p>{stats.autoReplied}</p>
                    </div>
                </div>
                    ...
                    <tbody>
                        {filteredQueries.length > 0 ? (
                            filteredQueries.map((query) => (
                                <tr key={query._id} className={`query-row ${query.status}`}>
